...
import { onTableChange, notifyTableChanged } from '../lib/refresh-bus'
...
export function Dashboard() {
  ...
  useEffect(() => {
    const unsubIdeas = onTableChange('ideas', () => {
      refreshIdeas()
      refreshIdeasList()
    })
    const unsubDrafts = onTableChange('content_items', (payload) => {
      if (payload?.newRecord?.status === 'draft') refreshDrafts()
      if (payload?.newRecord?.status === 'draft') refreshDraftsList()
    })
    const unsubReviews = onTableChange('content_items', (payload) => {
      if (payload?.newRecord?.status === 'review') refreshReviews()
      if (payload?.newRecord?.status === 'review') refreshReviewsList()
    })
    const unsubThemes = onTableChange('themes', () => {
      refreshThemes()
      refreshThemesList()
    })
  ...
  return (
    <div>
      ...
      <div className="grid-2">
        <div className="card">
          <div className="card-header">
            <div className="card-title">Drafts Queue</div>
            <Link to="/content-queue" className="btn btn-sm btn-ghost">
              View all <ArrowRight size={14} />
            </Link>
          </div>
          {draftsListLoading ? (
            ...
            <div className="empty-state" style={{ padding: 24 }}>
              <FileText size={32} />
              <h3>No drafts yet</h3>
              <p>Generate content to start building your queue</p>
              <Link to="/ai-writer" className="btn btn-primary btn-sm" style={{ marginTop: 12 }}>
                Generate content
              </Link>
            </div>
          ) : (
        ...
        <div className="card">
          ...
          {reviewsListLoading ? (
            ...
            <div className="empty-state" style={{ padding: 24 }}>
              <CheckSquare size={32} />
              <h3>All caught up</h3>
              <p>No content items awaiting review</p>
            </div>
          ) : (
            <table className="data-table">
              <thead><tr><th>Title</th><th>Type</th><th>Score</th></tr></thead>
              <tbody>
                {(reviews as any[])?.slice(0, 3).map((item: any) => (
                  <tr key={item.id}>
                    <td style={{ fontWeight: 500 }}>{item.title || 'Untitled'}</td>
                    <td><span className="tag">{item.content_type}</span></td>
                    <td>{item.ai_score ? `${Math.round(item.ai_score * 100)}%` : '—'}</td>
